...
class Splitter(QObject):
    progress_split_updated = pyqtSignal(float)

    def __init__(self):
        super().__init__()

    def split_video_ffmpeg(self, input_video_paths, scene_list, output_file_template, video_name,
        ...
        try:
            ...
            for i, (start_time, end_time) in enumerate(scene_list):
                ...
                if progress_bar:
                    ...
                    num_frame = num_frame + duration.get_frames()
                    self.progress_split_updated.emit(num_frame / total_frames * 100)
            if progress_bar:
                print('')
                logging.info('Average processing speed %.2f frames/sec.',
                             float(total_frames) / (time.time() - processing_start_time))
